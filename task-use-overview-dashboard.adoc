...
[role="lead"]
使用概述仪表板检查FSx for ONTAP文件系统的运行状况和容量。您可以筛选数据、查看容量建议、调整SSD容量设置以及查看性能趋势，以帮助决定在哪里放置卷以及何时扩展容量。
...
Workload Factory 会自动为每个 FSx for ONTAP 文件系统运行容量建议。您可以通过筛选 Clusters 表来查找包含建议的文件系统。
...
同时更改多个文件系统的 SSD 容量设置，以便在集群中应用相同的设置。

.关于此任务
选择多个文件系统进行批量编辑时：

* 如果所有选定的文件系统都使用相同的设置，则这些设置在编辑对话框中显示为默认值。
* 如果任何选定的文件系统处于"无"模式，则会在对话框中填充默认值。
* 如果所选文件系统的参数值不同，则会在对话框中填充默认值。
...
* **设置明确的阈值**：选择适合您的工作负载和容量计划的 SSD 管理阈值。
* **使用筛选器**：应用区域和帐户筛选器，将重点放在要审核的基础架构部分。
* **快速查看建议**：按 **Last SSD increase timestamp** 或描述筛选集群表，以查找具有容量建议的文件系统。检查并采取相应措施，以避免空间耗尽。
* **选择正确的模式**：如果要自己批准容量更改，请使用 **Recommend** 模式。如果您希望系统自动管理容量，请使用 **Automation** 模式。
* **观察决策点**：检查容量图表上的决策点，了解系统如何做出决策，并确认设置是否符合您的需求。
* **批量编辑**：管理多个文件系统时，请使用批量编辑来保持容量设置的一致性。
